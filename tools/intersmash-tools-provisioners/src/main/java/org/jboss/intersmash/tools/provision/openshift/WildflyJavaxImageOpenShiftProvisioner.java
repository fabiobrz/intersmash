...
/**
 * Class deploys a WildFly Jakarta EE 8/EAP 7.z application based on
 * {@link org.jboss.intersmash.tools.application.openshift.WildflyJavaxImageOpenShiftApplication}
 */
@Slf4j
public class WildflyJavaxImageOpenShiftProvisioner implements OpenShiftProvisioner<WildflyJavaxImageOpenShiftApplication> {

	private final WildflyJavaxImageOpenShiftApplication application;
	private FailFastCheck ffCheck = () -> false;

	public WildflyJavaxImageOpenShiftProvisioner(@NonNull WildflyJavaxImageOpenShiftApplication application) {
		this.application = application;
	}

	@Override
	public WildflyJavaxImageOpenShiftApplication getApplication() {
		return application;
	}
	...
	private ApplicationBuilder getAppBuilder() {
		...
		if (BinarySource.class.isAssignableFrom(buildInput.getClass())) {
			...
			BinaryBuildFromFile build = new BinaryBuildFromFile(
					IntersmashConfig.wildflyJakartaEe8ImageURL(),
	...
	private void deployImage() {
		...
		final String serverHomeDirectory = WildflyImageOpenShiftApplication.RuntimeVariant.Wildfly
				.equals(application.getRuntimeVariant()) ? "wildfly" : "eap";
		...
		appBuilder.deploymentConfig().podTemplate().container()
				.addReadinessProbe()
				.createExecProbe("/bin/bash", "-c", String.format("/opt/eap/bin/readinessProbe.sh", serverHomeDirectory));
		...
		if (!application.getCliScript().isEmpty()) {
			...
			appBuilder.deploymentConfig()
					...
					.container()
					.addVolumeMount("jboss-cli", "/opt/eap/extensions", false);
		}
