...
class IntersmashExecutionConditionTest {
	...
	}, targets = { Intersmash.Target.OpenShift })
	...
	@Intersmash(value = {
			@Service(Application.class)
	}, targets = { Intersmash.Target.Kubernetes })
	...
	@Intersmash(value = {
			@Service(Application.class)
	}, targets = { Intersmash.Target.Kubernetes, Intersmash.Target.OpenShift })
	class BothTargetTestClass {
	...
	@Test
	void evaluateExecutionCondition_targetingBothTestIsEnabledWhenBothTargetsAreSupported() {
		// Arrange
		systemProperties.set("intersmash.junit5.execution.targets", ALL_SUPPORTED.stream().collect(Collectors.joining(",")));
		XTFConfig.loadConfig();
		ExtensionContext extensionContext = new EmptyExtensionContext(BothTargetTestClass.class);
		// Act
		ConditionEvaluationResult conditionEvaluationResult = INTERSMASH_EXECUTION_CONDITION
				.evaluateExecutionCondition(extensionContext);
		// Assert
	...
	@Test
	void evaluateExecutionCondition_targetingBothTestIsDisabledWhenJustOneTargetsIsSupported() {
		// Arrange
		systemProperties.set("intersmash.junit5.execution.targets", Intersmash.Target.OpenShift);
		XTFConfig.loadConfig();
		ExtensionContext extensionContext = new EmptyExtensionContext(BothTargetTestClass.class);
		// Act
		ConditionEvaluationResult conditionEvaluationResult = INTERSMASH_EXECUTION_CONDITION
