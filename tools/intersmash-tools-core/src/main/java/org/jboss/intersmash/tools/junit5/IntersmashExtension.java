...
public class IntersmashExtension implements BeforeAllCallback, AfterAllCallback, TestInstancePostProcessor {
	...
	public void afterAll(ExtensionContext extensionContext) {
		...
		if (IntersmashConfig.skipUndeploy()) {
			...
			if (IntersmashExtensionHelper.isIntersmashTargetingOpenShift(extensionContext) && !IntersmashConfig.isOcp3x(OpenShifts.admin())) {
				operatorCleanup();
			}
	...
	private void injectServiceUrl(Object o, ExtensionContext extensionContext) throws Exception {
		...
		for (Field field : annotatedFields) {
			...
			Provisioner provisioner = IntersmashExtensionHelper.getProvisioners(extensionContext).get(serviceUrl.value().getName());
