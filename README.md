...
Although Intersmash is designed to allow executions on different platforms, we fully focus on OpenShift support currently while Kubernetes support is being introduced. Support for other platforms (Bare-metal etc.) could be added later on demand.
...
OpenShift cluster, a Kubernetes one, or both.
The `@Intersmash` annotation exposes the `target` field to allow for such configuration, with a comma-separated list 
...
environment, then such target must be configured through the `intersmash.junit5.execution.targets` property.   
